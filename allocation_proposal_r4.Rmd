...
idps <- read_csv("./data/idp_proj.csv") %>% 
  clean_names() %>% 
...
(16961605 - 13230378) / 16961605
```


Applying this methodology to currently available data, yields a unionwide PIN of 16,961,605. This is the total number of persons who are moderately and severely food insecure. This is a 22% increase from the 2022 PIN of 13,230,378. 
...
r4 %>% 
  ...
  left_join(idps %>% 
               mutate(state = recode(state, 
                        ...
                        "Shan (South)" = "Shan")) %>% 
              group_by(state) %>% 
              # IDP projections from UNHCR
              summarise(IDPs = sum(id_ps_projection)), 
            by = "state") %>% 
...
nat <- pin %>% 
  ...
  mutate(caseload_pc = caseload / sum(caseload, na.rm = TRUE), 
         PIN = caseload_pc * insecure_persons + IDPs, 
