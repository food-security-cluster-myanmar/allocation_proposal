...
Applying this methodology to currently available data, yields a unionwide PIN of `r sum(states$PIN, na.rm = TRUE) %>% format(big.mark = ",")`. This is the total number of persons who are moderately and severely food insecure. This is a `r round((sum(pin$fs_pin, na.rm = TRUE) - sum(states$PIN, na.rm = TRUE)) / sum(pin$fs_pin, na.rm = TRUE) * 100, digits = 2)`% decrease from the 2022 PIN of `r sum(states$PIN, na.rm = TRUE) %>% format(big.mark = ",")`. 
